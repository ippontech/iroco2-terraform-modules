...
func (db *MysqlService) CheckRoleExists(role types.Role) (bool, error) {
	var exists bool
	query := `SELECT EXISTS(SELECT 1 FROM mysql.user WHERE user = ?);`
	log.Debugf("checking if role %s exists", role)
...
func (db *MysqlService) CreateRole(role types.Role) error {
